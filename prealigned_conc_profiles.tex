...
\begin{frame}{Using DMAPS to Order Concentration Profiles}
	...
	\centering
  \begin{tikzpicture}
  	\node (unordered) {\includegraphics[width=0.4\textwidth]{data_unordered}};
  	\node[right of=unordered, node distance=0.5\textwidth] (ordered) {\includegraphics[width=0.4\textwidth]{data_ordered_DMAPS}};
    \draw [->] (unordered.east) -- (ordered.west) node[above,midway, text width=0.4\textwidth] {Ordered using the first DMAPS coordinate};    
  \end{tikzpicture}
  
  The developmental dynamics are smooth.
